...
class UserControllerSecurityIntegrationTest {
    @Autowired private com.safezone.user.repository.UserRepository userRepository;
    ...
    void setupTestUser() {
        ...
        com.safezone.user.entity.User user =
                com.safezone.user.entity.User.builder()
                        .username("testuser")
    ...
    void getUserById_publicAccess() throws Exception {
        mockMvc.perform(get("/api/v1/users/" + testUserId)).andExpect(status().isOk());
    }
    ...
    void addRole_requiresAuth() throws Exception {
        mockMvc.perform(post("/api/v1/users/1/roles/ADMIN")).andExpect(status().isUnauthorized());
    }
