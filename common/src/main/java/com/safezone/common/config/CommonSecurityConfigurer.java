 ...
 *
 * <p>SonarQube S4502 justification: CSRF is ignored for `/api/**` endpoints because: - Stateless
 * JWT authentication (Authorization header, no cookies/sessions) - SessionCreationPolicy.STATELESS
 * - No login forms or browser-based authentication - Gateway layer disables credentials (no
 * cross-origin cookies)
 *
 * <p>If cookies/sessions or `setAllowCredentials(true)` are introduced, remove this exception and
 * re-enable CSRF protection immediately.
 */
public final class CommonSecurityConfigurer {

    private CommonSecurityConfigurer() {}

    public static HttpSecurity applyDefaultSecurity(
            HttpSecurity http,
                        ...
                        csrf ->
                                csrf.requireCsrfProtectionMatcher(
                                        req -> {
                                            ...
                                            // Protect only when cookies are present (session-based
                                            // or cookie auth).
